"use client";

import { motion } from "framer-motion";
import { MessageSquare, Sparkles, Mic } from "lucide-react";
import { useRouter, usePathname } from "next/navigation";
import { cn } from "@/lib/utils";
import { useChat } from "./ChatProvider";

export default function ChatFAB() {
    const router = useRouter();
    const pathname = usePathname();
    const { toggleRecording, isRecording } = useChat();

    // Hide on chat routes and auth routes
    const isChatRoute = pathname?.startsWith("/chat/");
    const isAuthRoute = pathname === "/login" || pathname === "/register";

    if (isChatRoute || isAuthRoute) return null;

    return (
        <motion.div
            initial={{ scale: 0, opacity: 0, y: 50 }}
            animate={{ scale: 1, opacity: 1, y: 0 }}
            transition={{ type: "spring", stiffness: 400, damping: 25 }}
            className="fixed bottom-8 right-8 z-[100] flex flex-col gap-4"
        >
            {/* Voice Assistant Trigger */}
            <motion.button
                whileHover={{ scale: 1.1, x: -5 }}
                whileTap={{ scale: 0.9 }}
                onClick={toggleRecording}
                className={cn(
                    "group relative p-4 rounded-2xl border border-white/20 shadow-2xl transition-all overflow-hidden",
                    isRecording ? "bg-primary text-primary-foreground" : "bg-card text-foreground"
                )}
            >
                <div className="relative z-10 flex items-center justify-center">
                    <Mic className={cn("h-6 w-6", isRecording && "animate-pulse")} />
                </div>
                {!isRecording && (
                    <div className="absolute inset-0 bg-gradient-to-tr from-primary/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                )}

                {/* Tooltip */}
                <span className="absolute right-full mr-4 top-1/2 -translate-y-1/2 px-3 py-1.5 rounded-lg bg-card border border-border shadow-xl text-[8px] font-black uppercase tracking-wider text-foreground opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
                    Voice_Assistant
                </span>
            </motion.button>

            {/* AI Chat Trigger */}
            <button
                onClick={() => router.push("/chat")}
                className={cn(
                    "relative group p-6 rounded-[2rem] bg-foreground text-background shadow-[0_20px_50px_rgba(0,0,0,0.3)] dark:shadow-[0_20px_50px_rgba(255,255,255,0.05)] transition-all overflow-hidden border border-white/20",
                    "hover:ring-8 hover:ring-primary/10"
                )}
            >
                {/* Neural Pulse Effect */}
                <motion.div
                    animate={{ scale: [1, 1.2, 1], opacity: [0.3, 0.1, 0.3] }}
                    transition={{ repeat: Infinity, duration: 3 }}
                    className="absolute inset-0 bg-primary pointer-events-none"
                />

                {/* Icons */}
                <div className="relative flex items-center justify-center">
                    <MessageSquare className="h-7 w-7 transition-all duration-500 group-hover:rotate-[360deg] group-hover:scale-0 group-hover:opacity-0" />
                    <Sparkles className="h-7 w-7 absolute inset-0 scale-0 opacity-0 group-hover:scale-110 group-hover:opacity-100 transition-all duration-500 text-primary" />
                </div>

                {/* Tooltip */}
                <span className="absolute right-full mr-4 top-1/2 -translate-y-1/2 px-3 py-1.5 rounded-lg bg-card border border-border shadow-xl text-[8px] font-black uppercase tracking-wider text-foreground opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
                    Open_Advisor
                </span>
            </button>
        </motion.div>
    );
}
